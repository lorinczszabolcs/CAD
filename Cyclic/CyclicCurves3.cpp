...
namespace cagd
{
    GLdouble CyclicCurve3::_CalculateNormalizingCoefficients(GLuint n)
    {
        if (!n)
        {
            return 0.0;
    ...
    GLvoid CyclicCurve3::_CalculateBinomialCoefficients(GLuint n, TriangularMatrix<GLdouble> &bc)
    {
        bc.ResizeRows(n + 1);

        bc(0, 0) = 1.0;

        for (GLuint r = 1; r <= n; ++r)
        {
            bc(r, 0) = 1.0;
            bc(r, r) = 1.0;
    ...
    CyclicCurve3::CyclicCurve3(GLuint n, GLenum data_usage_flag):
        ...
        _n(n),
        _c_n(_CalculateNormalizingCoefficients(n)),
    ...
    GLboolean CyclicCurve3::BlendingFunctionValues(GLdouble u, RowMatrix<GLdouble> &values) const
    {
        ...
        {
            values[i] = _c_n * pow (1.0 + cos(u - i * _lambda_n), (GLuint)_n);
        }
