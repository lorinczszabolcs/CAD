...
GenericCurve3* LinearCombination3::GenerateImage(GLuint max_order_of_derivatives, GLuint div_point_count, GLenum usage_flag) const
{
    ...
    if (!result)
    {
        cout << "Can not create Generic Curve!\n";
        return nullptr;
    }
    ...
    for (GLuint i = 0; i < div_point_count; i++)
    {
        ...
        if (!CalculateDerivatives(max_order_of_derivatives, u, derivatives))
        {
            cout << "Can not calculate derivatives!\n";
            delete result, result = nullptr;
            return result;
        }

        result->_derivative.SetColumn(i,derivatives);
    }
