...
GLboolean TensorProductSurface3::RenderData(GLenum render_mode) const
{
    ...
    const GLuint OFFSET = _data.GetColumnCount() * _data.GetRowCount();

    for(GLuint i = 0; i < _data.GetColumnCount(); i++)
    {
        glDrawArrays(render_mode, OFFSET + i * _data.GetRowCount(), _data.GetRowCount());
    }
...
GLboolean TensorProductSurface3::UpdateVertexBufferObjectsOfData(GLenum usage_flag)
{
    ...
    glBindBuffer(GL_ARRAY_BUFFER, _vbo_data);
    glBufferData(GL_ARRAY_BUFFER, 2 * _data.GetColumnCount() * _data.GetRowCount() * 3 * sizeof(GLfloat), 0, _data_usage_flag);
